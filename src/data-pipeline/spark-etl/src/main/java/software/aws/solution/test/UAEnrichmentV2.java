...
import static software.aws.solution.test.ETLRunner.*;
    ...
    public Dataset<Row> transform(final Dataset<Row> dataset) {
        UserDefinedFunction udfEnrichUserAgent = udf(enrich(), DataTypes.createStructType(
                new StructField[]{
                        ...
                        DataTypes.createStructField(Constant.DEVICE_UA, ModelV2.STR_TO_STR_MAP_TYPE, true),
                }
        ));
        Dataset<Row> datasetUa = dataset.withColumn(DatasetUtil.UA_ENRICH, udfEnrichUserAgent.apply(col(Constant.UA)));

        Dataset<Row> enrichedDataset = datasetUa
                .withColumn(Constant.DEVICE_UA_BROWSER, col(DatasetUtil.UA_ENRICH).getField(Constant.DEVICE_UA_BROWSER))
                .withColumn(Constant.DEVICE_UA_BROWSER_VERSION, col(DatasetUtil.UA_ENRICH).getField(Constant.DEVICE_UA_BROWSER_VERSION))
                .withColumn(Constant.DEVICE_UA_OS, col(DatasetUtil.UA_ENRICH).getField(Constant.DEVICE_UA_OS))
                .withColumn(Constant.DEVICE_UA_OS_VERSION, col(DatasetUtil.UA_ENRICH).getField(Constant.DEVICE_UA_OS_VERSION))
                .withColumn(Constant.DEVICE_UA_DEVICE, col(DatasetUtil.UA_ENRICH).getField(Constant.DEVICE_UA_DEVICE))
                .withColumn(Constant.DEVICE_UA_DEVICE_CATEGORY, col(DatasetUtil.UA_ENRICH).getField(Constant.DEVICE_UA_DEVICE_CATEGORY))
                .withColumn(Constant.DEVICE_UA, col(DatasetUtil.UA_ENRICH).getField(Constant.DEVICE_UA))
