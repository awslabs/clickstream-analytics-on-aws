...
public class ETLRunner {
    ...
    public void run() {
        ...
        log.info(ContextUtil.JOB_NAME_PROP + ":" + System.getProperty(ContextUtil.JOB_NAME_PROP));
        log.info(ContextUtil.WAREHOUSE_DIR_PROP + ":" + System.getProperty(ContextUtil.WAREHOUSE_DIR_PROP));
    ...
    public Dataset<Row> readInputDataset(final boolean checkModifiedTime) {
        ...
        StructType inputDataSchema = DataTypes.createStructType(
                new StructField[]{
                        ...
                        DataTypes.createStructField(TransformerV3.CLIENT_TIMESTAMP, DataTypes.LongType, true),
        ...
        Dataset<Row> dataset = spark.read()
                ...
                .withColumn(TransformerV3.INPUT_FILE_NAME, input_file_name());
        ...
        List<Row> inputFiles = dataset.select(col(TransformerV3.INPUT_FILE_NAME).alias("fileName")).distinct().collectAsList();
    ...
    protected long writeResult(final String outputPath, final Dataset<Row> dataset, final TableName tbName) {
        ...
        if ("json".equalsIgnoreCase(runConfig.getOutPutFormat())) {
            ...
            int outPartitions = Integer.parseInt(System.getProperty(ContextUtil.OUTPUT_COALESCE_PARTITIONS_PROP, "-1"));
