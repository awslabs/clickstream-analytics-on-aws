...
import software.aws.solution.test.model.ModelV2;
import software.aws.solution.test.transformer.BaseTransformerV3;
...
import software.aws.solution.test.transformer.MaxLengthTransformerV2;
...
public class TransformerV3 extends BaseTransformerV3 {
    ...
    public static Dataset<Row> aggUserDataset(final Dataset<Row> userDataSet, final String info) {
        ...
        Dataset<Row> profileSetUserPropsDataset = userDataSet.filter(
                col(Constant.USER_PROPERTIES).isNotNull()
                        .and(col(latestEventName).equalTo(DatasetUtil.EVENT_PROFILE_SET))
        );
        ...
        Dataset<Row> userFirstDataSet = userDataSet.filter(
                col(firstEventName).isin(
                        ...
                        DatasetUtil.EVENT_SESSION_START,
                        DatasetUtil.EVENT_SCREEN_VIEW));
    ...
    public Dataset<Row> extractUser(final Dataset<Row> eventDataset, final Dataset<Row> convertedDataset) {
        Dataset<Row> userDataset = convertedDataset.select(expr("dataOut.user.*"))
                .select(ModelV2.toColumnArray(ModelV2.getUserFields()));
        ...
        DatasetUtil.PathInfo pathInfo = DatasetUtil.addSchemaToMap(newUserAggDataset, tableName, TABLE_VERSION_SUFFIX_V3);
        log.info("tableName: {}", tableName);
        log.info("pathInfo - incremental: " + pathInfo.getIncremental() + ", full: " + pathInfo.getFull());

        // save new (append)
        String path = DatasetUtil.saveIncrementalDatasetToPath(pathInfo.getIncremental(), newUserAggDataset);

        // read back full from incremental path
        Dataset<Row> fullUserDataset = DatasetUtil.readDatasetFromPath(eventDataset.sparkSession(), path, ContextUtil.getUserKeepDays());
        ...
        DatasetUtil.saveFullDatasetToPath(pathInfo.getFull(), fullUserAggDataset);
        ...
        return addProcessInfo(MaxLengthTransformerV2.runMaxLengthTransformerForUserV2(userDatasetFinal));
    }

    @Override
    public  Dataset<Row> getCleanedDataset(final Dataset<Row> dataset) {
        Dataset<Row> cleanedDataset = this.cleaner.clean(dataset, DatasetUtil.DATA_SCHEMA_V2_FILE_PATH);
        cleanedDataset = cleanedDataset.drop(DatasetUtil.DATA)
                .withColumnRenamed(DATA_STR, DatasetUtil.DATA);
    ...
    public TransformerNameEnum getName() {
        return TransformerNameEnum.CLICKSTREAM;
    }
    ...
    public Dataset<Row> extractSessionFromEvent(final Dataset<Row> eventDataset) {
        ...
        Dataset<Row> sessionDatasetWeb = sessionDataset
                .filter((col(Constant.EVENT_NAME).equalTo(DatasetUtil.EVENT_SESSION_START))
        ...
        Dataset<Row> sessionDatasetMobile = sessionDataset
                .filter((col(Constant.EVENT_NAME).isin(
                        DatasetUtil.EVENT_SCREEN_VIEW,
                        DatasetUtil.EVENT_USER_ENGAGEMENT,
        ...
        Dataset<Row> sessionDatasetAgg = sessionDatasetWebAgg
                ...
                .select(ModelV2.toColumnArray(ModelV2.getSessionFields()));
        return addProcessInfo(MaxLengthTransformerV2.runMaxLengthTransformerForSession(sessionDatasetAgg));
    }
