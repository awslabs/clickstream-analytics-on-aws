...
import static software.aws.solution.test.TransformerV3.DATA_STR;
...
import static software.aws.solution.test.util.ContextUtil.DATA_FRESHNESS_HOUR_PROP;
import static software.aws.solution.test.util.ContextUtil.WAREHOUSE_DIR_PROP;
import static software.aws.solution.test.util.ContextUtil.JOB_NAME_PROP;
...
public class Cleaner {
    ...
    public Dataset<Row> clean(final Dataset<Row> datasetInput, final String schemaFile) {
        ...
        if (!DatasetUtil.hasColumn(datasetInput, INPUT_FILE_NAME)) {
    ...
    private Dataset<Row> processDataColumnSchema(final Dataset<Row> dataset, final String schemaFile) {
        ...
        options.put("columnNameOfCorruptRecord", DatasetUtil.CORRUPT_RECORD);
        Dataset<Row> rowDataset = dataset
                .withColumn(DATA_STR, col(DatasetUtil.DATA).cast(StringType))
                .withColumn(DatasetUtil.DATA, from_json(col(DatasetUtil.DATA), dataType, options));
    ...
    private Dataset<Row> processCorruptRecords(final Dataset<Row> dataset) {
        Column corruptCondition = col("data").getItem(DatasetUtil.CORRUPT_RECORD).isNotNull()
        ...
        if (corruptedDatasetCount > 0) {
            ...
            corruptedDataset.write().partitionBy(DatasetUtil.JOB_NAME_COL)
        ...
        return dataset.filter(not(corruptCondition))
                .drop(col("data").getItem(DatasetUtil.CORRUPT_RECORD));
    }
