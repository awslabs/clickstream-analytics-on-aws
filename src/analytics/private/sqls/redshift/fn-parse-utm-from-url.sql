...
def parse_utm_from_url(url, referrer=None):
    ...
    if utm_source is None and referrer is not None:
        ...
        referrer_host_and_path = referrer.split("//")[1].split("?")[0]
        query_string = referrer.split("?")[1] if len(referrer.split("?")) > 1 else None
        ...
        for referrer_url in find_url_list:
            ...
            if category_item is not None:
                utm_source = category_dict["source"]
                source_category = category_dict["category"]
                query_keys = category_dict["params"]
