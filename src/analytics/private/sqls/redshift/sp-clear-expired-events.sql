...
BEGIN
    ...
    EXECUTE 'SELECT event_timestamp FROM {{schema}}.{{table_event}} ORDER BY event_timestamp DESC LIMIT 1' INTO latest_timestamp_record;
    CALL {{schema}}.{{sp_clickstream_log_non_atomic}}(log_name, 'info', 'get event_timestamp = ' || latest_timestamp_record.event_timestamp || ' from {{schema}}.{{table_event}}');
    ...
    ELSE
        DELETE FROM {{schema}}.{{table_event}} WHERE event_timestamp < (latest_timestamp_record.event_timestamp - retention_range_days * 24 * 3600 * 1000);
