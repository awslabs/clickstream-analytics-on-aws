CREATE OR REPLACE PROCEDURE {{dbName}}.{{schema}}.clickstream_dashboard_engagement_page_screen_view_detail_sp (day date) 
 LANGUAGE plpgsql
AS $$ 
DECLARE 
...
BEGIN
    ...
    DELETE FROM {{dbName}}.{{schema}}.clickstream_dashboard_engagement_page_screen_view_detail where event_date = day;

    INSERT INTO {{dbName}}.{{schema}}.clickstream_dashboard_engagement_page_screen_view_detail (
        event_date,
        aggregation_type,
        aggregation_dim,
        user_id,
