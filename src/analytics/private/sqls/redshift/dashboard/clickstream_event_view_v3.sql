CREATE MATERIALIZED VIEW {{scheam}}.clickstream_event_view_v3
BACKUP YES
...
SELECT
  ...
  DATE_TRUNC('hour', CONVERT_TIMEZONE('{{timezone}}', event_timestamp)) AS event_date_hour,
  e.event_id,
  e.user_pseudo_id,
  u.user_id,
  e.event_time_msec,
  ...
  CASE WHEN event_name = '_page_view' or event_name = '_screen_view' then e.session_id else null end as view_session_indicator,
  CASE WHEN event_name = '_page_view' or event_name = '_screen_view' then e.event_id else null end as view_event_indicator
FROM {{scheam}}.event_v2 as e
JOIN {{scheam}}.session_m_view as s ON e.user_pseudo_id = s.user_pseudo_id AND e.session_id = s.session_id
