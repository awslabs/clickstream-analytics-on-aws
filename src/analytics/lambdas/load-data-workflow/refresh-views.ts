...
import { CLICKSTREAM_ACQUISITION_ACTIVE_USER_COMPARE_MV, CLICKSTREAM_ACQUISITION_DAY_USER_VIEW_CNT_MV, CLICKSTREAM_ACQUISITION_MONTH_TRAFFIC_SOURCE_USER_MV, CLICKSTREAM_ACQUISITION_NEW_USER_COMPARE_MV, CLICKSTREAM_EVENT_VIEW_NAME, CLICKSTREAM_SESSION_M_MAX_VIEW_NAME, CLICKSTREAM_SESSION_M_VIEW_NAME, CLICKSTREAM_USER_M_MAX_VIEW_NAME, CLICKSTREAM_USER_M_VIEW_NAME } from '@aws/clickstream-base-lib';
    ...
    for (let rawAppId of appIds) {
      ...
      if (refreshInfo === undefined || Date.now() - refreshInfo.lastRefreshTime >= Number(REFRESH_INTERVAL_MINUTES) * 60 * 1000) {
        const sqlStatementForApp = `
          REFRESH MATERIALIZED VIEW ${schema}.${CLICKSTREAM_USER_M_MAX_VIEW_NAME};
          REFRESH MATERIALIZED VIEW ${schema}.${CLICKSTREAM_SESSION_M_MAX_VIEW_NAME};
          ...
          REFRESH MATERIALIZED VIEW ${schema}.${CLICKSTREAM_ACQUISITION_ACTIVE_USER_COMPARE_MV};

          REFRESH MATERIALIZED VIEW ${schema}.${CLICKSTREAM_ACQUISITION_NEW_USER_COMPARE_MV};
          REFRESH MATERIALIZED VIEW ${schema}.${CLICKSTREAM_ACQUISITION_DAY_USER_VIEW_CNT_MV};
          REFRESH MATERIALIZED VIEW ${schema}.${CLICKSTREAM_ACQUISITION_ACTIVE_USER_COMPARE_MV};
          REFRESH MATERIALIZED VIEW ${schema}.${CLICKSTREAM_ACQUISITION_MONTH_TRAFFIC_SOURCE_USER_MV};
        `;
