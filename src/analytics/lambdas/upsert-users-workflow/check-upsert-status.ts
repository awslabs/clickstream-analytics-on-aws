...
export const queryUpsertLog = async (appId: string) => {
  ...
  try {
    ...
    const queryId = await executeStatementsWithWait(redshiftDataApiClient, [querySqlStatement], serverlessRedshiftProps, provisionedRedshiftProps);
    ...
    await executeStatementsWithWait(redshiftDataApiClient, [delSqlStatement], serverlessRedshiftProps, provisionedRedshiftProps);
    return {
      detail: {
        appId: schema,
