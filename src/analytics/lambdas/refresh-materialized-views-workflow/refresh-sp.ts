...
export const handler = async (event: RefreshSpEvent) => {
  ...
  try {
    ...
    const spName = event.detail.taskName;
    // const refreshDate = event.originalInput.refreshDate;
    // TODO mingtong
    // sqlStatements.push(`CALL ${schema}.${spName}('${refreshDate}')`);
    sqlStatements.push(`REFRESH MATERIALIZED VIEW ${schema}.${spName};`);
    ...
    logger.info(`Refresh sp for app: ${schema} finished, spName: ${spName}`);
    return {
      detail: {
        appId: schema,
        queryId: queryId,
