...
import { getAppRegistryStackTags, getStackName, getTemplateUrlFromResource, mergeIntoPipelineTags, mergeIntoStackTags } from '../common/utils';
...
export async function generateWorkflow(pipeline: IPipeline, resources: CPipelineResources) {
  return {
    ...
    Workflow: await generateAppRegistryWorkflow(pipeline, resources),
  };
}

async function generateAppRegistryWorkflow(pipeline: IPipeline, resources: CPipelineResources): Promise<WorkflowState> {
...
function getStateCallback(pipeline: IPipeline): { BucketName: string; BucketPrefix: string } {
  if (!stackWorkflowS3Bucket) {
    throw new ClickStreamBadRequestError('Stack Workflow S3Bucket can not empty.');
  }
  return {
    BucketName: stackWorkflowS3Bucket,
    BucketPrefix: `clickstream/workflow/${pipeline.executionDetail?.name ?? pipeline.status?.executionDetail?.name}`,
  };
}
