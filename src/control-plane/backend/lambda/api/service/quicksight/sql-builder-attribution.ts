  ...
  if (params.computeMethod === ExploreComputeMethod.EVENT_CNT) {
    ...
    attributionDataSql = `
      ...
      model_data_with_user as (
        ...
        from joined_base_data
        join ${params.schemaName}.${USER_TABLE} as u
        on joined_base_data.user_pseudo_id = u.user_pseudo_id
      ),
      attribution_data as (
        select 
          t_event_name
...
export function buildSQLForPositionModel(params: AttributionSQLParameters): string {
  ...
  const modelDataSql = `
    ...
    model_data as (
      select
          joined_base_data.*
          ,case when joined_base_data.row_seq = model_base_data.row_seq_max then ${params.modelWeights![2]}
  ...
  if (params.computeMethod === ExploreComputeMethod.EVENT_CNT) {
    attributionDataSql = `
      attribution_data as (
        select
           joined_base_data.t_event_name
          ,count(t_event_id) as contribution
          ,case when joined_base_data.row_seq = model_base_data.row_seq_max then 'last_touch'
                when joined_base_data.row_seq = model_base_data.row_seq_min then 'first_touch'
                else 'middle_touch' end as touch_point_type
          end as contribution
        from
          joined_base_data
          join model_base_data on joined_base_data.user_pseudo_id = model_base_data.user_pseudo_id 
