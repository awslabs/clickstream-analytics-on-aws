...
describe('SQL Builder test', () => {
  ...
  select
    day,
    count(distinct u_id_0) as add_button_click,
    count(distinct u_id_1) as note_share,
    count(distinct u_id_2) as note_export
  from
    final_table
  group by
    day`.trim().replace(/ /g, ''),
  ...
  select
    day,
    count(distinct e_id_0) as add_button_click,
    count(distinct e_id_1) as note_share,
    count(distinct e_id_2) as note_export
  from
    final_table
  group by
    day`.trim().replace(/ /g, ''),
