...
def get_hash_for(event):
    event_str = json.dumps(event, default=to_json)
    hash_object = hashlib.md5(event_str.encode(), usedforsecurity=False)
    return hash_object.hexdigest()[:7]


def generate_ip_by_country(country_code, iprange):
    # current_dir = os.path.abspath(os.path.dirname(__file__))
    # db_path = os.path.join(current_dir, 'GeoLite2-Country.mmdb')
    # with Reader(db_path) as reader:
    #     ip_address = generate_random_ip(iprange[0], iprange[1])
    #     response = reader.country(ip_address)
    #     while response.country.iso_code != country_code:
    #         ip_address = generate_random_ip(iprange[0], iprange[1])
    #         response = reader.country(ip_address)
    #     return ip_address
