 ...
 */
import { DescribeStatementCommand, ExecuteStatementCommand, ExecuteStatementInput, RedshiftDataClient } from '@aws-sdk/client-redshift-data';
...
describe('Custom resource - Create schemas for applications in Redshift database', () => {
  ...
  test('Updated schemas in Redshift provisioned cluster', async () => {
    redshiftDataMock
      .callsFake(input => {
        if (input as ExecuteStatementInput) {
          if (input.Sql.includes('CREATE SCHEMA IF NOT EXISTS app1')
          && input.Sql.includes('CREATE SCHEMA IF NOT EXISTS app2')) {
            return { Id: 'Id-1' };
          }
          throw new Error('Sql is not expected');
        }
        return {};
      });
    redshiftDataMock.on(DescribeStatementCommand).resolves({ Status: 'FINISHED' });
