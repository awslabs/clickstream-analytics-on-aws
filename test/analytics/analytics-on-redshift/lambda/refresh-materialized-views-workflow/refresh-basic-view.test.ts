...
describe('Lambda - do refresh job in Redshift Serverless', () => {
  ...
  test('Executed Redshift refresh basic mv command', async () => {
    ...
    expect(resp).toEqual({
      detail: {
        appId: refreshBasicViewEvent.detail.appId,
        taskName: refreshBasicViewEvent.detail.taskName,
        queryId: exeuteId,
        taskType: 'MV',
      },
    });
    expect(redshiftDataMock).toHaveReceivedCommandWith(ExecuteStatementCommand, {
      ...
      Sql: `REFRESH MATERIALIZED VIEW ${refreshBasicViewEvent.detail.appId}.${refreshBasicViewEvent.detail.taskName};`,
    });
  });
