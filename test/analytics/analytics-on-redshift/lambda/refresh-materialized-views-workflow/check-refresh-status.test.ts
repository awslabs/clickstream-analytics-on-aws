...
import { handler, CheckRefreshViewStatusEvent } from '../../../../../src/analytics/lambdas/refresh-materialized-views-workflow/check-refresh-status';
...
describe('Lambda - check the refresh status in Redshift Serverless', () => {
  ...
  test('Check refresh status with response FINISHED', async () => {
    ...
    expect(resp).toEqual({
      detail: {
        status: StatusString.FINISHED,
        completeRefreshView: checkRefreshViewStatusEvent.detail.taskName,
      },
      appId: checkRefreshViewStatusEvent.detail.appId,
      waitTimeInfo: {
        loopCount: 1,
        waitTime: 30,
      },
