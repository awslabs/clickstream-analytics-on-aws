...
describe('Lambda - do refresh in Redshift Serverless', () => {
  ...
  test('Executed Redshift refresh basic mv command', async () => {
    ...
    expect(resp).toEqual({
      detail: {
        appId: refreshSpEvent.detail.appId,
        taskName: refreshSpEvent.detail.taskName,
        queryId: exeuteId,
        taskType: 'SP',
      },
    });
    expect(redshiftDataMock).toHaveReceivedCommandWith(ExecuteStatementCommand, {
      ...
      Sql: `REFRESH MATERIALIZED VIEW ${refreshSpEvent.detail.appId}.${refreshSpEvent.detail.taskName};`,
    });
  });
