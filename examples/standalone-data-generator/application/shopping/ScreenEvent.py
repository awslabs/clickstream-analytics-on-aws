...
def get_enter_new_screen_events(user, event, page):
    ...
    if user.current_page_type != '':
        ...
        if engagement_time > 1000:
            engagement_event = ShoppingApp.clean_event(event)
            engagement_event['attributes']['_engagement_time_msec'] = engagement_time
            events.append(ShoppingApp.get_final_event(user, EventType.USER_ENGAGEMENT, engagement_event))
    # screen view
    screen_view_event = ShoppingApp.clean_event(event)
    ...
    events.append(ShoppingApp.get_final_event(user, event_type, screen_view_event))
    return events
...
def get_exit_app_events(user, event):
    ...
    if engagement_time > 1000:
        ...
        event['attributes']['_engagement_time_msec'] = engagement_time
        events.append(ShoppingApp.get_final_event(user, EventType.USER_ENGAGEMENT, event))
    # app end
    event = ShoppingApp.clean_event(event)
    events.append(ShoppingApp.get_final_event(user, EventType.APP_END, event))
    return events
...
def get_detail_page_events(event, user):
    ...
    if next_page == Page.DETAIL:
        ...
        events.append(ShoppingApp.get_final_event(user, EventType.BEGIN_CHECKOUT, ShoppingApp.clean_event(event)))
    return events, next_page
...
def get_profile_page_events(event, user):
    ...
    if next_page == Page.LOGIN:
        if user.is_login:
            ...
            events.append(ShoppingApp.get_final_event(user, EventType.LOGOUT, ShoppingApp.clean_event(event)))
    user.current_timestamp += random.randint(3, 10) * 1000
    return events, next_page
...
def get_login_page_events(event, user):
    ...
    if next_page == ShoppingScreen.Page.MAIN:
        ...
        if user.is_login_user:
            ...
            profile_set_event = ShoppingApp.clean_event(event)
            user_name = {
                "value": user.name,
                "set_timestamp": user.current_timestamp
            }
            ...
            events.append(ShoppingApp.get_final_event(user, EventType.PROFILE_SET, profile_set_event))

            events.append(ShoppingApp.get_final_event(user, EventType.LOGIN, ShoppingApp.clean_event(event)))
            user.is_login = True
