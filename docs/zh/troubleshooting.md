...
## 问题：在解决方案 Web 控制台遇到 "Internal Server Error"

**解决方法：**

请按照以下步骤排查此问题：

1. 在 CloudWatch Logs 中收集详细的错误日志。
2. 您可以选择以下其中一种方式：
    - 提交 AWS 支持工单，包括错误日志，以便进行进一步调查。
    - 在 GitHub 仓库的[问题][issues]页面为该解决方案开启一个新问题。

要收集运行解决方案 Web 控制台的 Lambda 函数的详细错误日志，请执行以下操作：

1. 导航到解决方案 Web 控制台的 CloudFormation 堆栈。
2. 点击**资源**选项卡，搜索**逻辑 ID** 为 `ApiFunction68` 的 Lambda 函数。
3. 点击**物理 ID**列中的链接，在函数页面打开 Lambda 函数。
4. 选择**监控**，然后选择**查看 CloudWatch Logs**。更多详情请参阅[使用控制台访问日志][lambda-logs]。
