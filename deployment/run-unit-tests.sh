...
# spark-etl
docker run -i --rm -v "$HOME/.gradle:/root/.gradle" -v `pwd`/src/data-pipeline/:/data --workdir /data \
...
# flink-etl
docker run -i --rm -v "$HOME/.gradle:/root/.gradle" -v `pwd`/src/streaming-ingestion/flink-etl/:/data --workdir /data \
